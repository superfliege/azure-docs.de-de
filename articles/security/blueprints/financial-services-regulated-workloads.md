...
Einen kurzen Überblick über die Funktionsweise dieser Lösung finden Sie in diesem [Video](https://aka.ms/fsiblueprintvideo), in dem die Bereitstellung erklärt und gezeigt wird.

## <a name="solution-components"></a>Lösungskomponenten

Die Architektur, die die Bereitstellungsfunktionen der Azure PCI-DSS-Compliance-Lösung nutzt, besteht aus folgenden Komponenten.
...
Der Administrator des Contoso Webstore sucht nach einer Lösung, die zur Erreichung seiner Ziele schnell bereitgestellt werden kann. Er verwendet diesen Proof of Concept (POC), um mit den Projektbeteiligten zu besprechen, wie mithilfe von Azure Finanzdaten gemäß den strengen Konformitätsanforderungen gesammelt, gespeichert und abgerufen werden können.

> Sie sind für die Durchführung angemessener Sicherheits- und Konformitätsprüfungen für jede Lösung verantwortlich, die mit der von dieser POC verwendeten Architektur erstellt wurde, da die Anforderungen je nach den Besonderheiten Ihrer Implementierung und Region unterschiedlich sein können. 
...
- Über das Administratorkonto ist es nicht möglich, Finanzdaten ohne Maskierung zu lesen. Alle Aktionen werden protokolliert.
...
- Mit dem Konto „sqladmin“ ist es nicht möglich, Finanzdaten ungefiltert anzuzeigen. Alle Aktionen werden protokolliert.
...
- [SSL-Auslagerung](/azure/application-gateway/application-gateway-ssl-portal) aktiviert
- [TLS v1.0 und v1.1](/azure/application-gateway/application-gateway-end-to-end-ssl-powershell) deaktiviert
...
- [Diagnoseprotokollierung](/azure/application-gateway/application-gateway-diagnostics) aktiviert
- [Benutzerdefinierte Integritätstests](/azure/application-gateway/application-gateway-create-gateway-portal)
- Das [Azure Security Center](https://azure.microsoft.com/services/security-center) und der [Azure Advisor](/azure/advisor/advisor-security-recommendations) bieten zusätzlichen Schutz und zusätzliche Benachrichtigungen. Azure Security Center stellt außerdem ein Reputationssystem bereit.
...
Im Contoso Webstore werden alle sensiblen Daten verschlüsselt, und mit Azure Key Vault werden Schlüssel verwaltet und das Abrufen von Karteninhaberdaten verhindert.
...
- Zur Verschlüsselung aller kundenseitigen Finanzdaten wird [SQL-TDE](/sql/relational-databases/security/encryption/transparent-data-encryption) verwendet.
...
- [TLS 1.0](/azure/app-service-web/app-service-app-service-environment-custom-settings) deaktiviert
- [TLS-Verschlüsselung](/azure/app-service-web/app-service-app-service-environment-custom-settings) geändert
...
- [SQL-Datenbank-Datenverkehr](/azure/app-service-web/app-service-app-service-environment-network-architecture-overview) zugelassen


#### <a name="jumpbox-bastion-host"></a>Jumpbox (Bastionhost)

Da die App Service-Umgebung geschützt und gesperrt ist, muss es einen Mechanismus geben, der es erlaubt, alle DevOps-Releases oder -Änderungen zu berücksichtigen, die notwendig sein könnten, so z.B. die Möglichkeit, die Web-App mit Kudu zu überwachen. Ein virtueller Computer ist hinter dem NAT-Lastenausgleich geschützt, wodurch über einen anderen Port als TCP-Port 3389 eine Verbindung mit der VM hergestellt werden kann. 
...
Kunden sind dafür verantwortlich, eine Kopie der [Matrix der Verantwortungsübersicht](https://aka.ms/fsiblueprintcrm) zu erhalten. In dieser Matrix sind die in der Verantwortung des Kunden liegenden FFIEC-Anforderungen und die Anforderungen skizziert, die in der Verantwortung von Microsoft Azure liegen.



## <a name="disclaimer-and-acknowledgments"></a>Haftungsausschluss und Softwarehinweise
...
- Die Lösung in diesem Dokument ist als eine grundlegende Architektur gedacht und darf nicht ungeändert für Produktionszwecke verwendet werden. Zur Erzielung von Konformität ist der Kunde verpflichtet, die finale Kundenlösung in Zusammenarbeit mit dem Prüfer zu überprüfen.  
