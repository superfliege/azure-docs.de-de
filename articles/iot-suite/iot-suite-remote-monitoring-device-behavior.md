...
Der Artikel [Grundlegendes zum Gerätemodellschema](iot-suite-remote-monitoring-device-schema.md) beschreibt das Schema, das ein simuliertes Gerätemodell definiert. Dieser Artikel bezieht sich auf zwei Arten der JavaScript-Datei, die das Verhalten eines simulierten Geräts implementiert:
...
Der Abschnitt [Simulation](iot-suite-remote-monitoring-device-schema.md#simulation) des Gerätemodellschemas definiert den internen Status eines simulierten Geräts:
...
Der Abschnitt [CloudToDeviceMethods](iot-suite-remote-monitoring-device-schema.md#cloudtodevicemethods) des Gerätemodellschemas definiert die Methoden, auf die ein simuliertes Gerät reagiert.
...
Es gibt zwei globalen Funktionen, die Sie verwenden können, um das Verhalten der Methode zu implementieren:
...
Im Artikel [Testen der Lösung mit simulierten Geräten](iot-suite-remote-monitoring-test.md) erfahren Sie, wie Sie den Gerätesimulationsdienst lokal ausführen können. Die lokale Ausführung des Diensts erleichtert das Debuggen Ihrer simulierten Geräte, bevor Sie sie in der Cloud bereitstellen.
...
Jetzt wissen Sie, wie Sie das Verhalten eines simulierten Geräts festlegen, und sollten als Nächstes das [Testen der Lösung mit simulierten Geräten](iot-suite-remote-monitoring-test.md) kennenlernen.
