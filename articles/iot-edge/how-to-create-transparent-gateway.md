...
Erstellen Sie das neue Gerätezertifikat.  `myGatewayCAName` darf **NICHT** mit dem Hostnamen Ihres Gateways identisch sein.  Andernfalls tritt bei der Clientzertifizierung für diese Zertifikate ein Fehler auf.
...
| Output | Dateiname |
| ------ | --------- |
| Gerätezertifikat | `certs/new-edge-device.cert.pem` |
| Privater Schlüssel des Geräts | `private/new-edge-device.cert.pem` |
| Gerätezertifikatkette | `certs/new-edge-device-full-chain.cert.pem` |
| IoT Hub-Besitzerzertifizierungsstelle | `certs/azure-iot-test-only.root.ca.cert.pem`  |
...
Standardmäßig wird mit den Beispielskripts keine Passphrase für den privaten Schlüssel des Geräts festgelegt. Wenn Sie eine Passphrase festlegen, fügen Sie den folgenden Parameter hinzu: `--device-ca-passphrase {passphrase}`.
