---
title: Erstellen einer ASP.NET-Web-App mit Azure Redis Cache | Microsoft-Dokumentation
description: In dieser Schnellstartanleitung erfahren Sie, wie Sie eine ASP.NET-Web-App mit Azure Redis Cache erstellen.
...
ms.locfileid: "38674885"
---
# <a name="quickstart-create-an-aspnet-web-app"></a>Schnellstart: Erstellen einer ASP.NET-Web-App 

## <a name="introduction"></a>Einführung

In dieser Schnellstartanleitung wird erörtert, wie Sie mit Visual Studio 2017 eine ASP.NET-Webanwendung erstellen und für Azure App Service bereitstellen. Die Beispielanwendung stellt eine Verbindung mit Azure Redis Cache her, um Daten aus dem Cache abzurufen zu speichern. Nach der Ausführung der in dieser Schnellstartanleitung beschriebenen Schritte verfügen Sie über eine funktionsfähige, in Azure gehostete Web-App, die Lese- und Schreibvorgänge in Azure Redis Cache ausführt.
...
In diesem Abschnitt aktualisieren Sie die Anwendung, um eine neue Ansicht zu unterstützen, in der ein einfacher Test für Azure Redis Cache angezeigt wird.
...
3. Das NuGet-Paket wird heruntergeladen und fügt die benötigten Assemblyverweise zu Ihrer Clientanwendung hinzu, um mithilfe des StackExchange.Redis-Cacheclients auf den Azure Redis Cache zuzugreifen. Installieren Sie das Paket `StackExchange.Redis.StrongName`, wenn Sie es vorziehen, eine Version der `StackExchange.Redis`-Clientbibliothek mit sicheren Namen zu verwenden.
...
2. Wählen Sie im Browser auf der Navigationsleiste **Azure Redis Cache-Test** aus.

3. Im folgenden Beispiel hat der `Message`-Schlüssel zuvor einen zwischengespeicherten Wert aufgewiesen, der im Portal über die Azure Redis Cache-Konsole festgelegt wurde. Die App hat diesen zwischengespeicherten Wert aktualisiert. Außerdem hat die App die Befehle `PING` und `CLIENT LIST` ausgeführt.
...
Wählen Sie in der Navigationsleiste **Azure Redis Cache-Test** aus, um den Cachezugriff zu testen.
...
Im nächsten Tutorial verwenden Sie Azure Redis Cache in einem realitätsnäheren Szenario zur Verbesserung der Leistung einer App. Sie werden diese Anwendung zum Zwischenspeichern von Leaderboard-Ergebnissen anhand des cachefremden Musters mit ASP.NET und einer Datenbank aktualisieren.
