...
An dieser Stelle kommen Parameter ins Spiel. Den Wert für diese Eigenschaften (im Fall der Ressourcengruppe die Eigenschaften „Name“ und „Standort“) können Sie mit Blueprint während der Zuweisung zu einem Abonnement statt bei der Definition der Blaupause festlegen. So können Sie eine Blaupause wiederverwenden, die eine Ressourcengruppe und andere Ressourcen innerhalb eines einzelnen Abonnements erstellt, ohne einen Konflikt zu verursachen.

## <a name="blueprint-parameters"></a>Blaupausenparameter

Über die REST-API können Parameter zusätzlich zu den einzelnen unterstützten Artefakten in der Blaupause selbst erstellt werden. Wenn ein Parameter in der Blaupause erstellt wird, kann er von den Artefakten in dieser Blaupause verwendet werden. Ein Beispiel ist das Präfix für das Benennen der Ressourcengruppe. Das Artefakt kann mithilfe des Blaupausenparameters einen „größtenteils dynamischen“ Parameter erstellen, der weiterhin bei der Zuweisung definiert werden kann, aber eine Konsistenz aufweist, die den Benennungsregeln der Organisation entspricht. Anweisungen hierzu finden Sie unter [Festlegen statischer Parameter: Parameter auf Blaupausenebene](#blueprint-level-parameter).
...
Auf diese Weise wird die unsichere Methode vermieden, Geheimnisse zusammen mit der Blaupause zu speichern, und der Einsatz sicherer Muster wird gefördert. Azure Blueprint erleichtert dies, indem beide secure-Parameter in einem _Artefakt_ der Resource Manager-Vorlage erkannt werden und bei der Blaupausenzuweisung bei jedem erkannten secure-Parameter nach den folgenden Key Vault-Eigenschaften gefragt wird:

- Key Vault-Ressourcen-ID
- Name des Key Vault-Geheimnisses
- Version des Key Vault-Geheimnisses

Der referenzierte Schlüsseltresor muss in demselben Abonnement vorliegen, dem auch die Blaupause zugewiesen wird. Außerdem muss auf der Seite **Zugriffsrichtlinien** des Schlüsseltresors die Option **Zugriff auf Azure Resource Manager für Vorlagenbereitstellung aktivieren** konfiguriert sein. Eine Anleitung zum Aktivieren dieses Features finden Sie unter [Key Vault – Aktivieren der Vorlagenbereitstellung](../../../managed-applications/key-vault-access.md#enable-template-deployment).
...
Ein Parameterwert, der in der Definition einer Blaupause definiert wird, wird als **statischer Parameter** bezeichnet. Der Grund hierfür liegt darin, dass das Artefakt bei jeder Verwendung der Blaupause mit diesem statischen Wert bereitgestellt wird. Im Beispiel der Ressourcengruppe ist dies zwar für den Namen der Ressourcengruppe nicht sinnvoll, möglicherweise aber für den Standort. In diesem Fall wird durch jede Zuweisung der Blaupause eine Ressourcengruppe an demselben Standort erstellt – unter dem jeweiligen Namen, der bei der Zuweisung angegeben wird. Dadurch können Sie auswählen, was Sie als erforderlich definieren und was bei der Zuweisung geändert werden kann.
...
In diesem Beispiel hat die Eigenschaft **principalIds** den Blaupausenparameter **owners** durch Angabe des Werts `[parameters('owners')]` verwendet. Die Festlegung eines Parameters für ein Artefakt über einen Parameter auf Blaupausenebene ist ein Beispiel für einen **statischen Parameter**, weil dieser nicht während der Blaupausenzuweisung festgelegt werden kann und bei jeder Zuweisung diesen Wert aufweist.

##### <a name="artifact-level-parameter"></a>Parameter auf Artefaktebene

Das Erstellen **statischer Parameter** für ein Artefakt ist ähnlich, akzeptiert jedoch einen direkten Wert anstelle der `parameters()`-Funktion. Im folgenden Beispiel werden zwei statische Parameter erstellt: **tagName** und **tagValue**. Der Wert für jedes Beispiel wird direkt bereitgestellt und verwendet keinen Funktionsaufruf.
...
Das Gegenteil eines statischen Parameters ist ein **dynamischer Parameter**. Dies ist ein Parameter, der nicht in der Blaupause, sondern bei jeder Zuweisung der Blaupause definiert wird. Im Beispiel der Ressourcengruppe ist es für den Ressourcengruppennamen sinnvoll, für jede Zuweisung der Blaupause einen anderen Namen bereitzustellen.
...
1. Klicken Sie mit der rechten Maustaste auf die Blaupause, die Sie zuweisen möchten, und wählen Sie **Blaupause zuweisen**, oder klicken Sie auf die Blaupause, die Sie zuweisen möchten, und klicken Sie dann auf die Schaltfläche **Blaupause zuweisen**.
...
**Dynamische Parameter** werden während der Zuweisung festgelegt, indem der gewünschte Wert direkt angegeben wird. Anstelle einer Funktion wie `parameters()` wird eine passende Zeichenfolge als Wert angegeben. Artefakte für eine Ressourcengruppe werden mit einem „Vorlagennamen“ und den Eigenschaften **name** und **location** definiert. Alle anderen Parameter für jedes enthaltene Artefakt werden unter **parameters** mit einem **\<Name\>**-**Wert**-Schlüsselpaar definiert. Wenn die Blaupause für einen dynamischen Parameter konfiguriert ist, der bei der Zuweisung nicht angegeben wird, tritt bei der Zuweisung ein Fehler auf.
