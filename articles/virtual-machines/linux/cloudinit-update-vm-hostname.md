...
Standardmäßig ist der Hostname mit dem VM-Namen identisch, wenn Sie einen neuen virtuellen Computer in Azure erstellen.  Um mit [az vm create](/cli/azure/vm#create) beim Erstellen einer VM in Azure ein cloud-init-Skript zum Ändern des Standardhostnamens auszuführen, geben Sie die cloud-init-Datei mit dem Schalter `--custom-data` an.  

Um den Upgradevorgang in Aktion zu sehen, erstellen Sie in Ihrer aktuellen Shell eine Datei mit dem Namen *cloud_init_hostname.txt* und fügen die unten angegebene Konfiguration ein. Erstellen Sie für dieses Beispiel die Datei in der Cloud Shell, nicht auf dem lokalen Computer. Dazu können Sie einen beliebigen Editor verwenden. Geben Sie `sensible-editor cloud_init_hostname.txt` ein, um die Datei zu erstellen und eine Liste der verfügbaren Editoren anzuzeigen. Wählen Sie #1 aus, um den Editor **Nano** zu verwenden. Stellen Sie sicher, dass die gesamte cloud-init-Datei richtig kopiert wird, insbesondere die erste Zeile.  
