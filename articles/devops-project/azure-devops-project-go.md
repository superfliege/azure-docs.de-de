...
Konfigurieren Sie mit dem **Azure DevOps-Projekt** für Ihre **Go**-Anwendung Continuous Integration (CI) und Continuous Delivery (CD).  Das Azure DevOps-Projekt erleichtert die Erstkonfiguration einer VSTS-Build- und -Releasepipeline.
...
1. Erstellen Sie ein **neues** kostenloses VSTS-Konto, oder wählen Sie ein **vorhandenes** Konto aus.  Wählen Sie für Ihr VSTS-Projekt einen **Namen** aus.  Wählen Sie Ihr **Azure-Abonnement**, den **Speicherort** und einen **Namen** für Ihre Anwendung aus.  Klicken Sie auf **Fertig**, wenn Sie fertig sind.

    ![Eingeben von VSTS-Informationen](_img/azure-devops-project-go/vstsazureinfo.png)
