...
Im folgenden Befehl wird die Azure Service Fabric-Befehlszeilenschnittstelle verwendet, um die aktualisierte Resource Manager-Vorlage *template.json* mithilfe der Ressourcengruppe *myResourceGroup* bereitzustellen:
...
1. Deaktivieren Sie die VM-Instanzen der alten VM-Skalierungsgruppe, damit Sie den Knoten entfernen können. Dieser Vorgang dauert möglicherweise etwas länger. Sie können alle gleichzeitig deaktivieren. Daraufhin werden sie in eine Warteschlange eingereiht. Warten Sie, bis alle Knoten deaktiviert sind. 
#### <a name="azure-powershell"></a>Azure PowerShell
...
```
#### <a name="azure-cli"></a>Azure-Befehlszeilenschnittstelle
Im folgenden Befehl wird die Azure Service Fabric-Befehlszeilenschnittstelle verwendet, um die Knoteninstanz *NTvm1_0* in der alten VM-Skalierungsgruppe *NTvm1* zu deaktivieren:
```CLI
sfctl node disable --node-name "_NTvm1_0" --deactivation-intent RemoveNode
```
2. Entfernen Sie die vollständige Skalierungsgruppe. Warten Sie, bis der Bereitstellungsstatus der Skalierungsgruppe erfolgreich geändert wurde.
#### <a name="azure-powershell"></a>Azure PowerShell
...
```
#### <a name="azure-cli"></a>Azure-Befehlszeilenschnittstelle
Im folgenden Befehl wird die Azure Service Fabric-Befehlszeilenschnittstelle verwendet, um die gesamte Skalierungsgruppe *NTvm1* aus der Ressourcengruppe *myResourceGroup* zu entfernen:
...
Im folgenden Befehl wird die Azure Service Fabric-Befehlszeilenschnittstelle zum Entfernen des Lastenausgleichs *LB-myCluster-NTvm1* für die alte Skalierungsgruppe aus der Ressourcengruppe *myResourceGroup* verwendet:
...
Im folgenden Befehl wird die Azure Service Fabric-Befehlszeilenschnittstelle verwendet, um die DNS-Einstellungen der öffentlichen IP-Adresse *LBIP-myCluster-NTvm1* abzurufen und die IP-Adresse zu entfernen:
...
Im folgenden Befehl wird die Azure Service Fabric-Befehlszeilenschnittstelle verwendet, um die DNS-Einstellungen der öffentlichen IP-Adresse *LBIP-myCluster-NTvm3* mit den DNS-Einstellungen der alten IP-Adresse, die im vorherigen Schritt abgerufen wurden, zu aktualisieren:
...
Im folgenden Befehl wird die Azure Service Fabric-Befehlszeilenschnittstelle verwendet, um Service Fabric über die Entfernung des Knotens *NTvm1_0* zu benachrichtigen:
