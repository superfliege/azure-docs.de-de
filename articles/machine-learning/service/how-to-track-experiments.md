---
title: Nachverfolgen von Experimenten und Trainingsmetriken – Azure Machine Learning | Microsoft-Dokumentation
...
## <a name="log-metrics-for-experiments"></a>Protokollieren von Metriken für Experimente
...
## <a name="set-up-the-workspace-and-experiment"></a>Einrichten des Arbeitsbereichs und des Experiments
Bevor Sie die Protokollierung hinzufügen und ein Experiment übermitteln, müssen Sie den Arbeitsbereich und das Experiment einrichten.
...
2. Erstellen Sie das Experiment.
...
Das Skript endet mit ```run.complete()```, wodurch angegeben wird, dass die Ausführung abgeschlossen ist.  Dies wird in der Regel Szenarien mit einem interaktiven Notebook verwendet.

## <a name="option-2-use-scriptrunconfig"></a>Option 2: Verwenden von ScriptRunConfig

**ScriptRunConfig** ist eine Klasse für das Einrichten der Konfigurationen für Skriptausführungen. Mit dieser Option können Sie Überwachungscode hinzufügen, um über den Abschluss informiert zu werden oder um ein visuelles Widget zur Überwachung zu erhalten.
...
1. Erstellen Sie ein Trainingsskript. Dies wird verwendet ```%%writefile%%```, um den Trainingscode als ```train.py``` in den Skriptordner zu schreiben.
...
* Anzeigen der Ausführungsdetails durch Übermittlung des Namens der Ausführung (in diesem Fall ```run```). Darüber gelangen Sie zum Experimentname, ID, Typ, Status, Detailseite, einem Link zum Azure-Portal und einem Link zur Dokumentation.
...
Der Link für die Ausführung leitet Sie direkt zur Detailseite für die Ausführung im Azure-Portal. Hier finden Sie alle Eigenschaften, nachverfolgten Metriken, Images und Diagramme, die im Experiment protokolliert werden. In diesem Fall protokollierten wir MSE und die Alphawerte.
  ...
  ![Screenshot vom Ausführungsdetails im Azure-Portal](./media/how-to-track-experiments/run-details-page-web.PNG)
