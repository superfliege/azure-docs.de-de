...
> [Jupyter Notebook abrufen](https://aka.ms/aml-packages/forecasting/notebooks/financial_forecasting)
...
Jetzt können Sie die Prognosefehler für das Testdataset berechnen. Sie können hier MAPE (Mean Absolute Percentage Error, Prozentsatz des absoluten Fehlers des Erwartungswerts) verwenden. MAPE ist der mittlere absolute Fehler relativ zu den tatsächlichen Umsatzwerten. Die ```calc_error```-Funktion bietet einige integrierte Funktionen für häufig verwendete Fehlermetriken. Sie können auch Ihre benutzerdefinierte Fehlerfunktion definieren und an das ```err_fun```-Argument übergeben.
...
Es ist sinnvoll, die Verteilung dieser Fehler über die 249 Zeitreihen in den Daten anzuzeigen. Um die Verteilung der Fehler anzuzeigen, verwenden Sie das `by`-Argument in `calc_error` zum Berechnen der Fehler für jede Reihe. Erstellen Sie dann einen Boxplot, um sie visuell darzustellen.
...
Insgesamt scheint das naive Modell bessere Vorhersagen trotz einiger Ausreißer zu treffen, die weniger genau sind. 

## <a name="build-machine-learning-models"></a>Erstellen von Machine Learning-Modellen
Zusätzlich zu herkömmlichen univariaten Modellen können Sie mit dem Azure Machine Learning-Paket für Vorhersagen auch Machine Learning-Modelle erstellen. J

Beginnen Sie für dieses Modell zunächst mit dem Erstellen von Features.
...
Die [RegressionForecaster](https://docs.microsoft.com/python/api/ftk.models.regressionforecaster.regressionforecaster)-Funktion dient als Wrapper für sklearn-Regressionskalkulatoren, damit diese für TimeSeriesDataFrame trainiert werden können. Die Vorhersagefunktion im Wrapper positioniert auch jede Gruppe (in diesem Fall das Geschäft) im gleichen Modell. Die Vorhersagefunktion kann ein Modell für eine Gruppe von Reihen lernen, die als vergleichbar eingestuft wurden und in einem Pool zusammengefasst werden können. Ein Modell für eine Gruppe von Reihen verwendet häufig die Daten aus längeren Reihen, um die Prognosen für kurze Reihen zu verbessern. Verwenden Sie das `Lasso`- und `RandomForest`-Modell direkt aus `scikit-learn`.  Sie können diese Modelle durch beliebige andere Modelle in der Bibliothek ersetzen, die Regression unterstützen. 
...
Das Machine Learning-Modell konnte die hinzugefügten Features und die Ähnlichkeiten zwischen Reihen nutzen, um eine bessere Vorhersagegenauigkeit zu erzielen.
...
Die Pipelines sind nicht für die Bereitstellung angepasst. Das Bewerten des bereitgestellten Webdiensts trainiert das Modell erneut und generiert Vorhersagen für neue Daten. 
