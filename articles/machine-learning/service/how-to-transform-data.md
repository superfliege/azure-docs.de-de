---
title: Transformieren von Daten mit dem Azure Machine Learning Data Prep SDK – Python
...
Verwenden Sie den Ausdruck `substring(start, length)`, extrahieren Sie das Präfix aus der Spalte „Fallnummer“, und fügen Sie die Daten in die neue Spalte „Fallkategorie“ ein.
...
Verwenden Sie den Ausdruck `substring(start)`, um nur die Zahl aus der Spalte „Fallnummer“ zu extrahieren, konvertieren Sie sie dann in den numeric-Datentyp, und fügen Sie sie in die neue Spalte „Fall-ID“ ein.
```
...
Das Azure Machine Learning Data Prep SDK kann fehlende Werte in den angegebenen Spalten imputieren. In diesem Beispiel laden Sie Werte für Breiten- und Längengrade und imputieren dann fehlende Werte in den Eingabedaten.
...
Im dritten Datensatz fehlen Werte für Breiten- und Längengrade. Um diese fehlenden Werte zu imputieren, können Sie `ImputeMissingValuesBuilder` verwenden, um ein festes Programm zu lernen. Sie können die Spalten entweder mit einem berechneten `MIN`-, `MAX`- oder `MEAN`-Wert oder einem `CUSTOM`-Wert imputieren. Wenn `group_by_columns` angegeben ist, werden fehlende Werte nach Gruppe imputiert, wobei `MIN`, `MAX`, und `MEAN` pro Gruppe berechnet werden.

Überprüfen Sie zuerst schnell den Wert `MEAN` der Spalte „Breitengrad“.
```
...
Der Breitengradwert `MEAN` sieht gut aus, Sie können damit also den Breitengrad imputieren. Der fehlende Längengradwert wird basierend auf externen Informationen mit 42 imputiert.
...
Zu den erweiterten Tools im Azure Machine Learning Data Prep SDK zählt die Möglichkeit, Spalten anhand von Beispielen für die gewünschten Ergebnisse abzuleiten. Auf diese Weise können Sie dem SDK ein Beispiel hinzufügen, damit es zur Erzielung der gewünschten Ableitung Code generieren kann.
...
Wie Sie sehen können, ist diese Datei recht einfach aufgebaut. Allerdings wird davon ausgegangen, dass Sie diese Datei mit einem Dataset verknüpfen müssen, in dem Datum und Uhrzeit im Format „Mar 10, 2018 | 2AM-4AM“ angegeben sind.

Sie können die Daten in das benötigte Format transformieren.
...
Der obige Code erstellt zunächst einen Generator für die abgeleitete Spalte. Sie haben ein Array der zu berücksichtigenden Quellspalten (`DATE`) und einen Namen für die neue Spalte angegeben, die hinzugefügt werden soll.

Im ersten Beispiel haben Sie die zweite Zeile (Index 1) übergeben und einen erwarteten Wert für die abgeleitete Spalte angegeben.

Zu guter Letzt haben Sie `builder.preview()` aufgerufen und können neben der Quellspalte die abgeleitete Spalte sehen. Das Format wirkt auf den ersten Blick in Ordnung, Sie sehen jedoch nur Werte für das gleiche Datum „Jan 1, 2015“.
...
|11,9|11/2/2015 4:54|Feb 1, 2015 4AM-6AM|

Hier können Sie ein Problem mit dem generierten Programm sehen: Das abgeleitete Programm, das ausschließlich auf dem oben angegebenen Beispiel basiert, hat das Datum im Format „Tag/Monat/Jahr“ analysiert, was in diesem Fall jedoch nicht gewünscht ist.

Um dieses Problem zu beheben, müssen Sie ein weiteres Beispiel angeben.
...
Nun verarbeiten die Zeilen „1/2/2015“ ordnungsgemäß als „Jan 2, 2015“. Wenn Sie sich jedoch den Bereich weiter unten in der abgeleiteten Spalte ansehen, können Sie sehen, dass am Ende der abgeleiteten Spalte keine Werte angezeigt werden. Um dies zu beheben, müssen Sie ein weiteres Beispiel für Zeile 66 angeben.
...
Die Werte wirken auf den ersten Blick zwar einwandfrei, Sie werden jedoch bemerken, dass sie nicht ganz das gewünschte Ergebnis zeigen. Sie müssen Datum und Uhrzeit mit „|“ trennen, um das richtige Format zu generieren.

Um dies zu beheben, können Sie ein weiteres Beispiel hinzufügen. Anstatt Werte in eine Zeile aus der Vorschau zu übergeben, erstellen Sie dieses Mal ein Wörterbuch mit einer Name-Wert-Spalte für den Parameter `source_data`.
...
Dies hatte eindeutig negative Auswirkungen, da nun die einzigen Zeilen, die Werte in der abgeleiteten Spalte enthalten, die Zeilen sind, die genau den angegebenen Beispielen entsprechen.
...
Sie können sehen, dass wir inkonsistente Beispiele angegeben haben. Um das Problem zu beheben, müssen wir die ersten drei Beispiele durch richtige Beispiele ersetzen (einschließlich „|“ zwischen Datum und Uhrzeit einfügen).

Hierfür müssen die fehlerhaften Beispiele gelöscht (entweder durch Übergeben von `example_row` aus dem Pandas-DataFrame oder durch Übergeben des Werts `example_id`) und neue Beispiele hinzugefügt werden.
...
Nun sehen die Daten einwandfrei aus, und wir können abschließend im Generator `to_dataflow()` aufrufen, wodurch ein Datenfluss zurückgegeben wird, dem die gewünschten abgeleiteten Spalten hinzugefügt wurden.
...
Alternativ können Sie den Ausdruck `ColumnSelector` verwenden, um Spalten zu löschen, die einem RegEx-Ausdruck entsprechen. In diesem Beispiel löschen wir alle Spalten, die dem Ausdruck `Column*|.*longitude|.*latitude` entsprechen.
...
Verwenden Sie den Ausdrucks-Generator `col`, geben Sie den Spaltennamen als Zeichenfolgenargument `col('column_name')` an, und erstellen Sie mit einem der Standardoperatoren (>, <, >=, <=, ==, !=) einen Ausdruck wie `col('Tip_amount') > 0`. Übergeben Sie abschließend den erstellten Ausdruck an die Funktion `Dataflow.filter`.
...
In bestimmten Szenarien ist die einfachste Lösung, Python-Code zu schreiben. Das SDK bietet drei Erweiterungspunkte, die Sie verwenden können.
...
Suchen Sie mithilfe eines Filters nach NULL-Werten. Füllen Sie nun die fehlenden Werte mit den gefundenen Werten aus.
...
Mithilfe einer praktischen Pandas-Funktion können Sie alle NULL-Werte durch „0“ ersetzen. Dieser Code wird nicht für alle Datasets gleichzeitig ausgeführt, sondern partitionsweise. Das heißt, dass dieser Code bei einem großen Dataset parallel ausgeführt werden kann, während die Laufzeit die Daten partitionsweise verarbeitet.
...
Erstellen Sie nun einen Python-Ausdruck, um das Dataset nur auf die Zeilen zu filtern, in denen „Hale“ nicht in der neuen Spalte `county_state` enthalten ist. Der Ausdruck gibt `True` zurück, wenn die Zeile beibehalten werden soll, und `False`, um die Zeile zu löschen.
