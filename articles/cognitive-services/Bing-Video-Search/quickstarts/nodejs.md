...
1. Erstellen Sie eine Funktion mit dem Namen `response_handler`, um eine JSON-Antwort von der API zu erhalten. Erstellen Sie eine Variable für den Antworttext. Fügen Sie die Antwort unter Verwendung von `response.on()` an, wenn ein `data`-Flag empfangen wird.
    ...
    2. Wenn `end` angegeben wird, verwenden Sie `response.on()`, um die Bing-bezogenen Header (beginnend mit `bingapis` oder `x-msedge-`) zu speichern. Analysieren Sie anschließend den JSON-Code mit `JSON.parse()`, konvertieren Sie ihn mit `JSON.stringify()` in eine Zeichenfolge, und geben Sie ihn aus.
