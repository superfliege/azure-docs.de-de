...
Erstellen Sie in Ihrer bevorzugten IDE oder Ihrem bevorzugten Editor ein neues Python-Projekt. Kopieren Sie anschließend den folgenden Codeausschnitt in Ihr Projekt in eine Datei namens `transliterate-text.py`.
...
Derzeit ist ein Endpunkt für die Textübersetzung verfügbar, der als `base_url` festgelegt ist. `path` legt die `transliterate`-Route fest und gibt die gewünschte Version der API (Version 3) an.
...
Sie können die Textübersetzungs-API nicht nur für die Transliteration von Text, sondern auch für Folgendes verwenden:
