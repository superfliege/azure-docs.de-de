---
title: Funktionsweise der Personalisierung
titleSuffix: Azure Cognitive Services
description: Hier erfahren Sie, wie Sie den Personalisierungcontainer mit den Konfigurationseinstellungen für den Docker Run-Befehl ausführen.
author: edjez
manager: nitinme
...
Die Einstellung `Billing` gibt den Endpunkt-URI der Ressource vom Typ _Personalisierung_ in Azure an, der zum Messen der Abrechnungsinformationen für den Container verwendet wird. Sie müssen einen Wert für diese Konfigurationseinstellung angeben, und bei dem Wert muss es sich um einen gültigen URI-Endpunkt für die Ressource vom Typ _Personalisierung_ in Azure handeln.

Diese Einstellung finden Sie hier:

* Azure-Portal: Übersicht der **Personalisierung** mit der Bezeichnung `Endpoint`
...
Die Container verwenden keine Eingabe- oder Ausgabeeinbindungen zum Speichern von Trainings- oder Dienstdaten. 

Die genaue Syntax für den Bereitstellungspunkt auf dem Host variiert je nach Betriebssystem des Hosts. Darüber hinaus ist es eventuell nicht möglich, auf den Bereitstellungspunkt auf dem [Hostcomputer](personalizer-container-howto.md#the-host-computer) zuzugreifen, wenn ein Konflikt zwischen den vom Docker-Dienstkonto verwendeten Berechtigungen und den für den Bereitstellungspunkt auf dem Host verwendeten Berechtigungen besteht. 

|Optional| NAME | Datentyp | BESCHREIBUNG |
|-------|------|-----------|-------------|
|Nicht zulässig| `Input` | Zeichenfolge | Wird von Personalisierungscontainern nicht verwendet.|
...
Die folgenden Beispiele verwenden die Konfigurationseinstellungen, um zu veranschaulichen, wie `docker run`-Befehle geschrieben und verwendet werden.  Nach dem Ausführen wird der Container so lange ausgeführt, bis Sie ihn [beenden](personalizer-container-howto.md#stop-the-container).

* **Zeilenfortsetzungszeichen:** In den Docker-Befehlen in den folgenden Abschnitten wird der umgekehrte Schrägstrich (`\`) als Zeilenfortsetzungszeichen für eine Bash-Shell verwendet. Ersetzen oder entfernen Sie diesen je nach den Anforderungen des Hostbetriebssystems. Das Zeilenfortsetzungszeichen für Windows ist beispielsweise das Caretzeichen `^`. Ersetzen Sie den umgekehrten Schrägstrich durch das Caretzeichen. 
* **Argumentreihenfolge:** Ändern Sie die Reihenfolge der Argumente nur, wenn Sie mit Docker-Containern sehr gut vertraut sind.

Ersetzen Sie den Wert in Klammern (`{}`) durch Ihre eigenen Werte:

| Platzhalter | Wert | Format oder Beispiel |
|-------------|-------|---|
|{BILLING_KEY} | Der Endpunktschlüssel der Ressource vom Typ „Personalisierung“ |xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx|
...
> [!IMPORTANT]
> Die Optionen `Eula`, `Billing` und `ApiKey` müssen angegeben werden, um den Container auszuführen, andernfalls wird der Container nicht gestartet.  Weitere Informationen finden Sie unter [Abrechnung](personalizer-container-howto.md#billing).
> Der ApiKey-Wert ist der **Schlüssel** von der Schlüsselseite der Azure-Ressource vom Typ „Personalisierung“. 

## <a name="personalizer-container-docker-examples"></a>Beispiele für Docker-Container für die Personalisierung

Im Folgenden finden Sie Docker-Beispiele für den Personalisierungscontainer. 
...
[Grundlegendes zur Verwendung des Docker-Containers](personalizer-container-howto.md)