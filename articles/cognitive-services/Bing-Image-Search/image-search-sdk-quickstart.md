...
Der [C#-Beispielquellcode für das Bing-Bildersuche-SDK](https://github.com/Azure-Samples/cognitive-services-dotnet-sdk-samples/tree/master/BingSearchv7/BingImageSearch) ist auf GitHub verfügbar.

## <a name="application-dependencies"></a>Anwendungsabhängigkeiten

Navigieren Sie in Visual Studio über den Projektmappen-Explorer zur Option `Manage NuGet Packages`, um mit dem SDK für die Bing-Bildersuche eine Konsolenanwendung einzurichten.  Fügen Sie das Paket `Microsoft.Azure.CognitiveServices.Search.ImageSearch` hinzu.

Beim Installieren des [NuGet-Pakets für die Bildersuche](https://www.nuget.org/packages/Microsoft.Azure.CognitiveServices.Search.ImageSearch/1.2.0) werden auch Abhängigkeiten installiert, z.B.:
* Microsoft.Rest.ClientRuntime
* Microsoft.Rest.ClientRuntime.Azure
* Newtonsoft.Json

## <a name="image-search-client"></a>Client für die Bildersuche
Um eine Instanz des `ImageSearchAPI`-Clients zu erstellen, fügen Sie using-Direktiven hinzu:
```
...
```
Analysieren Sie die Bildergebnisse, die von der letzten Abfrage zurückgegeben wurden:
...
Die folgende Konsolenanwendung führt die zuvor ausgeführte Abfrage „Canadian Rockies“ aus, um erst nach Ergebnisse zu suchen und dann das erste Bildauswertungstoken, die URL zur Miniaturansicht und die URL für den Bildinhalt auszugeben:
...
Die Beispiele für die Bing-Suche veranschaulichen verschiedene Features des SDK.  Fügen Sie der zuvor definierten Klasse `ImageSrchSDK` die folgenden Funktionen hinzu.

### <a name="search-using-a-filter"></a>Durchsuchen mithilfe eines Filters

Durchsuchen Sie Bilder für „studio ghibli“, und filtern Sie dabei nach animierten GIFs und Bildern im Breitformat. Prüfen Sie anschließend die Anzahl der Ergebnisse, und geben Sie den insightsToken-Parameter, die URL der Miniaturansicht und die URL des ersten Ergebnisses aus.
...
### <a name="trending-images"></a>Beliebte Bilder

Suchen Sie nach beliebten Bildern, und überprüfen Sie anschließend die Kategorien und Kacheln.
...
### <a name="image-details"></a>Bilddetails

Suchen Sie erst nach Bildern für „Degas“, und suchen Sie anschließend nach Details zum ersten Bild.
```
