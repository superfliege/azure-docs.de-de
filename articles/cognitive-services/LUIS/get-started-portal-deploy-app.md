...
description: Sobald die App bereit ist, Äußerungsvorhersagen an eine Clientanwendung wie einen Chatbot zurückzugeben, müssen Sie die App für den Vorhersageendpunkt bereitstellen. In dieser Schnellstartanleitung erfahren Sie, wie Sie eine Anwendung bereitstellen, indem Sie eine Vorhersageendpunktressource erstellen, der App die Ressource zuweisen, die App trainieren und die App veröffentlichen.
...
Sobald die App bereit ist, Äußerungsvorhersagen an eine Clientanwendung wie einen Chatbot zurückzugeben, müssen Sie die App für den Vorhersageendpunkt bereitstellen. 

In dieser Schnellstartanleitung erfahren Sie, wie Sie eine Anwendung bereitstellen, indem Sie eine Vorhersageendpunktressource erstellen, der App die Ressource zuweisen, die App trainieren und die App veröffentlichen. 
...
## <a name="create-endpoint-resource"></a>Erstellen einer Endpunktressource

Die Vorhersageendpunktressource wird im Azure-Portal erstellt. Diese Ressource darf nur für Endpunktvorhersageabfragen verwendet werden. Verwenden Sie diese Ressource nicht für das Erstellen von Änderungen an der App. 

1. Melden Sie sich am **[Azure-Portal](https://ms.portal.azure.com/)** an. 

1. Wählen Sie im oberen linken Bereich das grüne Pluszeichen (**+**) aus, suchen Sie im Marketplace nach `Cognitive Services`, und wählen Sie dann diese Option aus. 
...
1. Konfigurieren Sie das Abonnement mit den folgenden Einstellungen:
    ...
    |Standort|**USA, Westen**|Die Azure-Region für diese Ressource.|
...
1. Wählen Sie den Mandantennamen, den Abonnementnamen und den Ressourcennamen aus, und wählen Sie dann **Ressource zuweisen** aus. 
...
Sie sollten die App trainieren, sobald Sie bereit sind, sie zu testen. Sie sollten die App veröffentlichen, sobald die aktuell trainierte Version für Clientanwendungen über die Laufzeit des Vorhersageendpunkts verfügbar sein soll. 

1. Wenn Sie die App noch nicht trainiert haben, wählen Sie im Menü oben rechts die Option **Trainieren** aus.

1. Wählen Sie im Menü oben rechts die Option **Veröffentlichen** aus. Übernehmen Sie die Standardumgebungseinstellungen, und wählen Sie **Veröffentlichen** aus.

1. Wenn oben im Browserfenster die grüne Erfolgsbenachrichtigungsleiste angezeigt wird, wählen Sie den Link **zum Anzeigen der Liste der Endpunkte** aus. 
...
1. Daraufhin wird die Seite **Schlüssel und Endpunkteinstellungen** geöffnet. Im unteren Bereich der Seite befindet sich die Liste mit den zugewiesenen Ressourcen und entsprechenden Endpunkt-URLs. 

1. Wählen Sie die Endpunkt-URL aus, die Ihrem neuen Ressourcennamen zugeordnet ist. Daraufhin wird ein Webbrowser mit einer ordnungsgemäß aufgebauten URL geöffnet, um eine **GET**-Anforderung an die Laufzeit des Vorhersageendpunkts zu senden. 
...
1. Die Zeichenfolge `q=` am Ende der URL ist die Kurzform für **Abfrage** (query). Dort wird die Äußerung des Benutzers an die GET-Anforderung angefügt. Geben Sie nach `q=` die gleiche Benutzeräußerung ein, die am Ende des vorherigen Schnellstarts verwendet wurde:
    ...
    Die Browserantwort ist die gleiche JSON, die Ihre Clientanwendung erhält:
    ...
    Diese Antwort bietet Ihnen mehr Informationen als der Standardbereich **Test** im vorherigen Tutorial. Wenn im Testbereich derselbe Umfang an Informationen angezeigt werden soll, muss die App veröffentlicht werden. Wählen Sie dann im Testbereich die Option **Mit veröffentlichter Version vergleichen** aus. Verwenden Sie im Testbereich für die Veröffentlichung die Option **JSON-Ansicht anzeigen**, um dieselbe JSON wie im vorherigen Schritt anzuzeigen. Dadurch können Sie die aktuelle App, die Sie bearbeiten, mit der App vergleichen, die am Endpunkt veröffentlicht wird.
...
Wählen Sie nach Abschluss dieses Schnellstarts im oberen Navigationsmenü die Option **Meine Apps** aus. Aktivieren Sie dann in der Liste das Kontrollkästchen links neben der App, und wählen Sie auf der Kontextsymbolleiste über der Liste die Option **Löschen** aus. 
