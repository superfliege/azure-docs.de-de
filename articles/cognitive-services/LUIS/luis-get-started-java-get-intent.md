---
title: 'Tutorial: Informationen zum Aufrufen einer LUIS-App (Language Understanding Intelligent Service) mithilfe von Java | Microsoft-Dokumentation'
...
ms.locfileid: "36263719"
---
# <a name="tutorial-call-a-luis-endpoint-using-java"></a>Tutorial: Aufrufen eines LUIS-Endpunkts mithilfe von Java
...
4. Kehren Sie zurück zur [LUIS](luis-reference-regions.md)-Website, und melden Sie sich mit Ihrem Azure-Konto an. 
    ...
    [![](media/luis-get-started-java-get-intent/app-list.png "Screenshot der App-Liste")](media/luis-get-started-java-get-intent/app-list.png)
...
2. Fügen Sie die URL in ein Browserfenster ein, und drücken Sie die EINGABETASTE. Das im Browser angezeigte JSON-Ergebnis gibt an, dass LUIS die Absicht `HomeAutomation.TurnOn` und die Entität `HomeAutomation.Room` mit dem Wert `bedroom` erkennt.
    ...
    ![JSON-Ergebnis, das die Absicht „TurnOn“ erkennt](./media/luis-get-started-java-get-intent/turn-on-bedroom.png)
3. Ändern Sie den Wert des Parameters `q=` in der URL in `turn off the living room light`, und drücken Sie die EINGABETASTE. Das Ergebnis gibt jetzt an, dass LUIS die Absicht `HomeAutomation.TurnOff` und die Entität `HomeAutomation.Room` mit dem Wert `living room` erkannt hat. 
    ...
    ![JSON-Ergebnis, das die Absicht „TurnOff“ erkennt](./media/luis-get-started-java-get-intent/turn-off-living-room.png)


## <a name="consume-a-luis-result-using-the-endpoint-api-with-java"></a>Nutzen eines LUIS-Ergebnisses mithilfe der Endpunkt-API und Java 

Sie können mit Java auf die gleichen Ergebnisse zugreifen, die im vorherigen Schritt im Browserfenster angezeigt wurden. 
1. Kopieren Sie den folgenden Code, um eine Klasse in der IDE zu erstellen:
...
2. Ersetzen Sie den Wert der Variablen `SubscriptionKey` durch Ihren LUIS-Abonnementschlüssel.

3. Fügen Sie in Ihrer IDE Verweise zu den `httpclient`- und `httpcore`-Bibliotheken hinzu.
