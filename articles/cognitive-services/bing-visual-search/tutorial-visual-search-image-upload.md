...
Die Funktion `addProducts()` erstellt einen Link für alle zurückgegebenen Einkaufsquellenergebnisse. Dazu wird die Liste der `Products`-Aktionen durchlaufen und ein `<img>`-Tag an den jeweiligen äußeren `<div>`-Abschnitt angefügt.
