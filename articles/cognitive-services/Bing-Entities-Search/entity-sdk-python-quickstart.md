...
ms.locfileid: "55189713"
---
# <a name="quickstart-bing-entity-search-sdk-with-python"></a>Schnellstart: Bing-Entitätssuche-SDK mit Python

Das Entitätssuche-SDK enthält die Funktionalität der REST-API für Entitätsabfragen und das Analysieren von Ergebnissen.

Der [Beispielquellcode für das Bing-Entitätssuche-SDK für Python](https://github.com/Azure-Samples/cognitive-services-python-sdk-samples/blob/master/samples/search/entity_search_samples.py) ist auf GitHub verfügbar.
...
```
Installieren Sie Abhängigkeiten des Bing-Entitätssuche-SDK:
```
cd mytestenv
...
```
Erstellen Sie eine Instanz der `CognitiveServicesCredentials`. Instanziieren Sie den Client:
```
client = EntitySearchAPI(CognitiveServicesCredentials(subscription_key))
```
Suchen Sie nach einer einzelnen Entität (Gibralter), und geben Sie eine kurze Beschreibung aus:
```
...
```
Führen Sie eine Suche durch, und behandeln Sie nicht eindeutige Ergebnisse einer mehrdeutigen Abfrage (William Gates).
```
...
```
Suchen Sie nach einem einzelnen Store (Microsoft Store), und geben Sie die entsprechende Telefonnummer aus.
```
...
```
Suchen Sie nach einer Liste von Restaurants (Seattle restaurants), und geben Sie die entsprechenden Namen und dazugehörigen Telefonnummern aus.
```
...
```
Lösen Sie eine ungültige Anforderung aus, und sehen Sie sich die Fehlerantwort an.
```
