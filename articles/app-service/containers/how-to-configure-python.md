...
> [!NOTE]
> [Python in App Service unter Windows](https://docs.microsoft.com/visualstudio/python/managing-python-on-azure-app-service) ist veraltet und sollte nicht mehr verwendet werden.
>
...
Sie können das Startverhalten des Containers steuern, indem Sie einen benutzerdefinierten Gunicorn-Startbefehl angeben. Wenn Sie beispielsweise über eine Flask-App verfügen, deren Hauptmodul *hello.py* lautet, und das Flask-App-Objekt in dieser Datei den Namen `myapp` hat, lautet der Befehl wie folgt:
...
Wenn Sie einen Gunicorn-fremden Server (beispielsweise [aiohttp](https://aiohttp.readthedocs.io/en/stable/web_quickstart.html)) verwenden möchten, können Sie Folgendes ausführen:
...
Alternativ können Sie den Befehl am Stamm Ihres Projekts in einer Textdatei namens *startup.txt* (oder mit einem beliebigen anderen Namen) speichern. Stellen Sie diese Datei dann in App Service bereit, und geben Sie stattdessen diesen Namen im Feld **Startdatei** an. Mit dieser Option können Sie den Befehl in Ihrem Quellcoderepository verwalten, anstatt über das Azure-Portal.
