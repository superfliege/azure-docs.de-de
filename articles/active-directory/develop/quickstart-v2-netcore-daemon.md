---
title: Azure AD v2 .NET Core-Daemon | Microsoft-Dokumentation
description: Es wird beschrieben, wie für einen .NET Core-Prozess ein Zugriffstoken abgerufen und eine API aufgerufen werden kann, die mit einem Azure Active Directory v2.0-Endpunkt geschützt ist, indem die eigene Identität der App verwendet wird.
...
![Zeigt, wie die in diesem Schnellstart generierte Beispiel-App funktioniert](media/quickstart-v2-netcore-daemon/netcore-daemon-intro-updated.png)

## <a name="prerequisites"></a>Voraussetzungen

Für diese Schnellstartanleitung ist [.NET Core 2.1](https://www.microsoft.com/net/download/dotnet-core/2.1) erforderlich.
...
>
> 1. Navigieren Sie zur [Anwendungsregistrierung (Vorschau) im Azure-Portal](https://portal.azure.com/?Microsoft_AAD_RegisteredApps=true#blade/Microsoft_AAD_RegisteredApps/applicationsListBlade/quickStartType/DotNetCoreDaemonQuickstartPage/sourceType/docs).
...
> | Hinweis: ||
> |---------|---------|
> | `secret` | Der geheime Clientschlüssel, der für die Anwendung im Azure-Portal erstellt wird. |
...
> | `Authority`    | (Optional) Der STS-Endpunkt für den zu authentifizierenden Benutzer. Normalerweise <https://login.microsoftonline.com/{tenant}> für die öffentliche Cloud, wobei „{tenant}“ der Name Ihres Mandanten bzw. Ihre Mandanten-ID ist.|
> | `redirectUri`  | Die URL, an die Benutzer nach der Authentifizierung geleitet werden. In diesem Fall wird dieser Parameter nicht verwendet, da es sich um eine Konsolenanwendung bzw. nicht interaktive Anwendung handelt. |
> | `clientCredentials`  | Das Clientanmeldeinformationen-Objekt, in dem entweder das Geheimnis oder das Zertifikat enthalten ist. |
> | `userTokenCache`  | Instanz eines Tokencaches für den Benutzer. In diesem Fall ist dieser Wert null, weil diese App im Kontext der App und nicht im Benutzerkontext ausgeführt wird.|
> | `appTokenCache`  | Instanz eines Tokencaches für die App.|

Weitere Informationen finden Sie in der [Referenzdokumentation für `ConfidentialClientApplication`](https://docs.microsoft.com/dotnet/api/microsoft.identity.client.confidentialclientapplication.-ctor?view=azure-dotnet).

### <a name="requesting-tokens"></a>Anfordern von Token

Verwenden Sie die `AcquireTokenForClientAsync`-Methode, um ein Token mit der Identität einer App anzufordern:
