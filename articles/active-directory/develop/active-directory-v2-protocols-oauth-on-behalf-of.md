...
> [!IMPORTANT]
> Token, die per [impliziter Gewährung](active-directory-v2-protocols-implicit.md) beschafft werden, können nicht für den „Im Auftrag von“-Ablauf verwendet werden. Der Client wird bei Abläufen für die implizite Gewährung nicht authentifiziert (z.B. über einen geheimen Clientschlüssel). Daher sollte es nicht zugelassen werden, dass ein Bootstrap-Vorgang in ein anderes Token mit höherer Leistungsstärke durchgeführt wird.

Die folgenden Schritte entsprechen dem Im-Auftrag-von-Ablauf und werden anhand des folgenden Diagramms erläutert.

![Im-Auftrag-von-Ablauf von OAuth2.0](media/active-directory-protocols-oauth-on-behalf-of/active-directory-protocols-oauth-on-behalf-of-flow.png)
