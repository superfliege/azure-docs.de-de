---
title: "Erstellen einer benutzerdefinierten Route zum Weiterleiten des Netzwerkdatenverkehrs über ein virtuelles Netzwerkgerät – Azure CLI | Microsoft-Dokumentation"
...
Die Azure-CLI-Befehle sind unter Windows, Linux und macOS identisch. Es gibt jedoch Unterschiede bei der Skripterstellung für die Betriebssystemshells. Für die Skripts, die in den folgenden Schritten angegeben werden, ist die Installation und Ausführung von Azure CLI-Befehlen in einer Bash-Shell erforderlich. Sie können entweder die [Azure CLI auf Ihrem PC installieren und konfigurieren](/cli/azure/install-azure-cli?toc=%2fazure%2fvirtual-network%2ftoc.json) oder in Skripts auf die Schaltfläche **Ausprobieren** klicken, um diese in Azure Cloud Shell auszuführen.
...
2. Wenn Sie die Azure CLI auf Ihrem Computer ausführen, melden Sie sich mit dem Befehl `az login` bei Azure an. Wenn Sie Cloud Shell verwenden, werden Sie automatisch angemeldet.
3. Legen Sie die folgenden Variablen für die verbleibenden Schritte fest:
...
4. Erstellen Sie ein *DMZ*-Subnetz im virtuellen Netzwerk, das im vorbereitenden Schritt erstellt wurde:
...
5. Erstellen Sie die VM des virtuellen Netzwerkgeräts. Weisen Sie der Netzwerkschnittstelle, die durch die CLI erstellt wird, statische öffentliche und private IP-Adressen zu. Statische Adressen ändern sich während des VM-Lebenszyklus nicht. Beim virtuellen Netzwerkgerät kann es sich um eine VM handeln, auf der das Betriebssystem Linux oder Windows ausgeführt wird. Kopieren Sie zur Erstellung der VM das Skript für eines der Betriebssysteme, und fügen Sie es in die CLI ein. Wenn Sie eine Windows-VM erstellen, fügen Sie das Skript in einen Text-Editor ein, ändern Sie den Wert der Variable *AdminPassword*, und fügen Sie den geänderten Text in die CLI ein.
...
6. Aktivieren Sie die IP-Weiterleitung für die Netzwerkschnittstelle des virtuellen Netzwerkgeräts. Hierdurch wird verhindert, dass Azure die IP-Adresse der Quelle und des Ziels überprüft. Wenn Sie diese Einstellung nicht aktivieren, wird der Netzwerkdatenverkehr, der an eine andere IP-Adresse als die des Netzwerkadapters gesendet wird, von Azure verworfen.
...
7. Erstellen Sie eine Routingtabelle für das *öffentliche* Subnetz.
...
9. Ordnen Sie die Routingtabelle *myRouteTable-Public* dem *öffentlichen* Subnetz zu. Hierdurch leitet Azure den gesamten ausgehenden Datenverkehr aus dem Subnetz entsprechend der Routen in der Routingtabelle weiter. Eine Routingtabelle kann entweder keinem oder mehreren Subnetzen zugeordnet werden. Einem Subnetz kann hingegen entweder keine oder genau eine Routingtabelle zugeordnet werden.
...
10. Erstellen Sie die Routingtabelle für das *private* Subnetz.
...
11. Erstellen Sie eine Route, um über die VM des virtuellen Netzwerkgeräts den Datenverkehr vom *privaten* an das *öffentliche* Subnetz weiterzuleiten.
...
12. Ordnen Sie die Routingtabelle dem *privaten* Subnetz zu.
...
13. **Optional**: Erstellen Sie jeweils eine VM im öffentlichen und privaten Subnetz, und überprüfen Sie, ob der Datenverkehr zwischen den VMs über das virtuelle Netzwerkgerät weitergeleitet wird. Führen Sie hierzu die Schritte im Abschnitt [Überprüfen des Routings](#validate-routing) aus.
14. **Optional**: Führen Sie die Schritte im Abschnitt [Löschen von Ressourcen](#delete-resources) aus, um die Ressourcen zu löschen, die Sie in diesem Tutorial erstellen.
...
2. Klicken Sie im folgenden Feld auf die Schaltfläche **Ausprobieren**. Dadurch wird Azure Cloud Shell geöffnet. Melden Sie sich mit Ihrem [Azure-Konto](../azure-glossary-cloud-terminology.md?toc=%2fazure%2fvirtual-network%2ftoc.json#account) bei Azure an, wenn Sie dazu aufgefordert werden. Wenn Sie nicht über ein Azure-Konto verfügen, können Sie sich für eine [kostenlose Testversion](https://azure.microsoft.com/offers/ms-azr-0044p)registrieren. Azure Cloud Shell ist eine kostenlose Bash-Shell, für die Azure CLI vorinstalliert ist. 
    ...
    Klicken Sie im folgenden **Linux**- oder **Windows**-Skript auf die Schaltfläche **Kopieren**, und fügen Sie den Inhalt des Skripts in einen Text-Editor ein. Ändern Sie das Kennwort für die Variable *adminPassword*, und fügen Sie das Skript in Azure Cloud Shell ein. Führen Sie das Skript für das Betriebssystem aus, das Sie in Schritt 5 im Abschnitt [Erstellen von Routen und virtuellen Netzwerkgeräten](#create-routes-and-network-virtual-appliance) bei der Erstellung des virtuellen Netzwerkgeräts ausgewählt haben. 
