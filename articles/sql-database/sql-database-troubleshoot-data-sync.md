...
Die Synchronisierung lokaler Datenbanken, die dem Agent zugeordnet sind, ist über die Portalbenutzeroberfläche für die SQL-Datensynchronisierung nicht erfolgreich. Auf dem lokalen Computer, auf dem der Agent ausgeführt wird, enthält das Ereignisprotokoll Fehler vom Typ „System.IO.IOException“. Diese Fehler weisen darauf hin, dass auf dem Datenträger nicht genügend Speicherplatz zur Verfügung steht.
...
Dieses Problem tritt auf, weil der Client-Agent der SQL-Datensynchronisierung keine Anmeldeinformationen speichert.
...
2. Klicken Sie mit der rechten Maustaste auf **SQL-Datensynchronisierungs-Agent**, und wählen Sie **Beenden** aus.
3. Klicken Sie mit der rechten Maustaste auf **SQL-Datensynchronisierungs-Agent**, und wählen Sie **Eigenschaften** aus.
4. Klicken Sie im Fenster **Eigenschaften des SQL-Datensynchronisierungs-Agents** auf die Registerkarte **Anmelden**.
...
8. Klicken Sie im Fenster **Dienste** mit der rechten Maustaste auf den Dienst **SQL-Datensynchronisierungs-Agent**, und klicken Sie anschließend auf **Starten**.
...
3. Beenden Sie den **SQL-Datensynchronisierungsdienst**.
4. Starten Sie den **SQL-Datensynchronisierungsdienst** neu.  
...
-   Stellen Sie sicher, dass der SQL-Datensynchronisierungsdienst ausgeführt wird:  
    a. Suchen Sie im Menü **Start** nach **Dienste**.  
    b. Klicken Sie in den Suchergebnissen auf **Dienste**.  
    c. Suchen Sie den **SQL-Datensynchronisierungsdienst**.  
...
Datenbanken, die mindestens 45 Tage vom Dienst getrennt waren (gemessen ab dem Zeitpunkt, zu dem die Datenbank offline geschaltet wurde), werden von der SQL-Datensynchronisierung entfernt. Eine Datenbank, die online geschaltet wird, nachdem sie mindestens 45 Tage offline war, weist den Status **Veraltet** auf.
