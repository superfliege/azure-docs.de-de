...
* [SQL Server auf Azure Virtual Machines](https://azure.microsoft.com/services/virtual-machines/sql-server/): SQL Server, installiert und in der Cloud gehostet auf in Azure ausgeführten virtuellen Windows Server-Computern (Windows Server-VMs), auch als Infrastructure-as-a-Service (IaaS) bezeichnet.
  SQL Server auf virtuellen Azure-Computern ist die gleiche SQL Server-Software, die Sie auf Ihrem lokalen Server installieren würden – der einzige Unterschied besteht darin, dass dieser Server jetzt eine in Azure gehostete VM ist. SQL Server auf Azure ist für die Migration bestehender SQL Server-Anwendungen optimiert, und es stehen alle Versionen und Editionen von SQL Server zur Verfügung. Folglich haben Sie die Möglichkeit, bei vollständiger Kontrolle Ihrer SQL Server-Instanz und Ihres Windows-Betriebssystem, so viele Datenbanken wie nötig zu hosten und die Unterstützung zum Ausführen von datenbankübergreifenden Transaktionen zu nutzen.
...
|  | Teams, die möchten, dass Microsoft ihre Datenbankupgrades, Hochverfügbarkeit und Notfallwiederherstellung verwaltet. |Teams, die Hochverfügbarkeit, Notfallwiederherstellung und Patches für SQL Server konfigurieren und verwalten können. Dies wird durch einige bereitgestellte Automatisierungsfeatures erheblich vereinfacht. | |
