...
|  | Teams, die integrierte Hochverfügbarkeit, Notfallwiederherstellung und Upgrades für die Datenbank benötigen. | Es gilt dasselbe wie für SQL-Datenbank-Singletons und gruppierte Datenbanken. | Teams, die Hochverfügbarkeit, Notfallwiederherstellung und Patches für SQL Server konfigurieren, optimieren und verwalten können. Dies wird durch einige bereitgestellte Automatisierungsfeatures erheblich vereinfacht. | |
|  | Teams, die das zugrunde liegende Betriebssystem und Konfigurationseinstellungen nicht selbst verwalten möchten. | Es gilt dasselbe wie für SQL-Datenbank-Singletons und gruppierte Datenbanken. | Sie benötigen eine angepasste Umgebung mit uneingeschränkten Administratorrechten. | |
...
Derzeit sind für den Dienst **SQL-Datenbank** verschiedene Bereitstellungsoptionen und Tarife mit unterschiedlichen Preisen für Ressourcen verfügbar, die nach einem festen Stundensatz je nach ausgewähltem Tarif und ausgewählter Computegröße berechnet werden. Die neuesten Informationen zu aktuell unterstützten Diensttarifen, Computegrößen und Speichermengen finden Sie unter [DTU-basiertes Kaufmodell für Azure SQL-Datenbank](sql-database-service-tiers-dtu.md) und [V-Kern-basiertes Kaufmodell](sql-database-service-tiers-vcore.md).

- Bei SQL-Datenbank-Singletons können Sie aus einer Vielzahl von Tarifen – ab einem Preis von 5 USD pro Monat für „Basic“ – den Diensttarif auswählen, der Ihre Anforderungen erfüllt.
...
Außerdem wird Ihnen der ausgehende Internetdatenverkehr basierend auf den üblichen [Datenübertragungsraten](https://azure.microsoft.com/pricing/details/data-transfers/)berechnet. Sie können Tarif und Computegröße entsprechend den unterschiedlichen Durchsatzanforderungen Ihrer Anwendung dynamisch anpassen.
