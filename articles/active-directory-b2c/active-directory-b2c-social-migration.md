---
title: 'Azure Active Directory B2C: Migrieren von Benutzern mit Identitäten sozialer Netzwerke'
description: Erläutert grundlegende Konzepte für die Migration von Benutzern mit Identitäten sozialer Netzwerke zu Azure AD B2C mithilfe der Graph-API
...
* Abhängig vom Identitätsanbieter handelt es sich bei der **Benutzer-ID eines sozialen Netzwerks** um einen eindeutigen Wert für einen bestimmten Benutzer `per application` oder ein Entwicklungskonto. Konfigurieren Sie die Azure AD B2C-Richtlinie mit der gleichen Anwendungs-ID, die zuvor vom Anbieter des sozialen Netzwerks zugewiesen wurde. Oder verwenden Sie eine andere Anwendung `within the same development account`.
