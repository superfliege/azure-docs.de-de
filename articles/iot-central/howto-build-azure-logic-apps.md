...
1. Beginnen Sie mit dem [Erstellen einer Regel in IoT Central](howto-create-telemetry-rules.md). Nachdem Sie die Regelbedingungen gespeichert haben, klicken Sie auf die Kachel **Azure Logic Apps**. Klicken Sie auf **Im Azure-Portal erstellen**. Sie gelangen zum Azure-Portal, wo Sie dann eine neue Logik-App erstellen können. Möglicherweise müssen Sie sich bei Ihrem Azure-Konto anmelden.
...
> [!NOTE]
> Für **Update a device** (Gerät aktualisieren) und **Delete a device** (Gerät löschen) benötigen Sie die ID des vorhandenen Geräts, das Sie aktualisieren bzw. löschen möchten. Sie können die ID des IoT Central-Geräts über die Browser-URL abrufen. Diese ID ist **NICHT** mit der Geräte-ID identisch, die auf der Explorer-Seite angezeigt wird.
