...
Der Lebenszyklus eines Modulzwillings ist mit der entsprechenden [Modulidentität][lnk-identity] verknüpft. Modulzwillinge werden implizit erstellt und gelöscht, wenn in IoT Hub eine Modulidentität erstellt oder gelöscht wird.
...
> [!NOTE]
> Gemeldete Eigenschaften tragen zur Vereinfachung von Szenarien bei, in denen das Lösungs-Back-End den letzten bekannten Wert einer Eigenschaft abfragen soll. Verwenden Sie [Gerät-zu-Cloud-Nachrichten][lnk-d2c], wenn das Lösungs-Back-End Modultelemetriedaten in Form einer Reihe von Ereignissen mit Zeitstempel abfragen muss (beispielsweise eine Zeitreihe).
...
3. Das Lösungs-Back-End kann die Ergebnisse des Konfigurationsvorgangs auf vielen Modulen durch [Abfragen][lnk-query] des Modulzwillings nachverfolgen.
...
Die [Azure IoT-Geräte-SDKs][lnk-sdks] vereinfachen die Verwendung der oben genannten Vorgänge mit vielen Sprachen und Plattformen.
...
IoT Hub verwaltet den Zeitstempel der letzten Aktualisierung für jedes JSON-Objekt in den gewünschten und gemeldeten Eigenschaften des Modulzwillings. Zeitstempel verwenden UTC und sind im [ISO8601]-Format codiert: `YYYY-MM-DDTHH:MM:SS.mmmZ`.
Beispiel: 
...
* [Erste Schritte mit der Modulidentität und dem Modulzwilling von IoT Hub unter Verwendung eines .NET-Back-Ends und eines .NET-Geräts][lnk-module-twin-tutorial]
