...
5. Fügen Sie der **Program** -Klasse die folgenden Felder hinzu. Ersetzen Sie den Platzhalterwert durch die IoT Hub-Verbindungszeichenfolge aus [Senden von Telemetriedaten von einem Gerät an eine IoT Hub-Instanz ...](quickstart-send-telemetry-dotnet.md):
...
> [!NOTE]
> Der Einfachheit halber wird in diesem Lernprogramm keine Wiederholungsrichtlinie implementiert. Im Produktionscode sollten Sie Wiederholungsrichtlinien implementieren (etwa einen exponentiellen Backoff), wie im MSDN-Artikel zum [Behandeln vorübergehender Fehler](https://msdn.microsoft.com/library/hh680901(v=pandp.50).aspx)beschrieben.
> 
