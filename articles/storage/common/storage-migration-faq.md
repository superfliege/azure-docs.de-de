...
AzCopy nutzt die [Copy Blob-API](https://docs.microsoft.com/rest/api/storageservices/copy-blob), um die einzelnen Dateien im Container zu kopieren.  
...
**Fallen für das Kopieren von Daten zwischen zwei verschiedenen Dateifreigaben im selben Speicherkonto in der gleichen Region Gebühren an?**
...
2.  Nachdem Sie AzCopy unter Windows mithilfe des Installers installiert haben, öffnen Sie ein Eingabeaufforderungsfenster, und navigieren Sie zum Installationsordner von AzCopy auf Ihrem Computer. AzCopy wird standardmäßig in **%ProgramFiles(x86)%\Microsoft SDKs\Azure\AzCopy** oder **%ProgramFiles%\Microsoft SDKs\Azure\AzCopy** installiert.

3.   Führen Sie den folgenden Befehl aus, um den Container zu verschieben. Sie müssen den Text durch den tatsächlichen Wert ersetzen.   
     
            AzCopy /Source:https://sourceaccount.blob.core.windows.net/mycontainer1
            /Dest:https://destaccount.blob.core.windows.net/mycontainer2
            /SourceKey:key1 /DestKey:key2 /S

        - /source: Geben Sie den URI des Quellspeicherkontos (bis zum Container) an.  
        - /dest: Geben Sie den URI des Zielspeicherkontos (bis zum Container) an.  
        - /sourcekey: Geben Sie den Primärschlüssel des Quellspeicherkontos an. Sie können diesen Schlüssel im Portal kopieren, indem Sie das Speicherkonto auswählen.  
        - /destkey: Geben Sie den Primärschlüssel des Zielspeicherkontos an. Sie können diesen Schlüssel im Portal kopieren, indem Sie das Speicherkonto auswählen.
...
2.  Kopieren Sie die Inhalte aus dem ursprünglichen Blobcontainer mit [AzCopy](https://azure.microsoft.com/en-us/blog/azcopy-5-1-release/) in einen anderen Blobcontainer.

**Wie erstelle ich ein PowerShell-Skript zum Verschieben von Daten von einer Azure-Dateifreigabe in einen anderen Azure-Speicher?**

Verwenden Sie AzCopy, um die Daten von einer Azure-Dateifreigabe in einen anderen Azure-Speicher zu verschieben. Weitere Informationen finden Sie unter [Übertragen von Daten mit AzCopy unter Windows](storage-use-azcopy.md) und [Übertragen von Daten mit AzCopy unter Linux](storage-use-azcopy-linux.md).

**Wie lade ich große CSV-Dateien in den Azure-Speicher hoch?**
...
**Wie kann ich lokal gespeicherte Daten in Azure-Speicher kopieren?**
...
**Wie kann ich lokal gespeicherte Daten in den Azure-Dateidienst verschieben?**
...
Es gibt keine Sicherungslösung. Azure Files unterstützt jedoch auch das asynchrone Kopieren. Sie können also Dateien von einer Freigabe in eine andere Freigabe (innerhalb eines Speicherkontos oder in einem anderen Speicherkonto) oder in einen Blobcontainer (innerhalb eines Speicherkontos oder in einem anderen Speicherkonto) kopieren.
...
Wenn Sie ein Speicherkonto erstellen, wählen Sie die primäre Region für das Konto aus. Die sekundäre Region wird basierend auf der primären Region ausgewählt und kann nicht geändert werden. Informationen finden Sie unter [Azure Storage-Replikation](storage-redundancy.md).
...
Nach dem Aktivieren der Verschlüsselung im Speicherkonto werden die vorhandenen Daten nicht verschlüsselt. Um die vorhandenen Daten zu verschlüsseln, müssen Sie die Daten erneut in das Speicherkonto hochladen.  Gehen Sie dazu folgendermaßen vor:

Kopieren Sie mithilfe von AzCopy die Daten in ein anderes Speicherkonto, und verschieben Sie sie anschließend zurück in ein Speicherkonto. Sie können auch die [Verschlüsselung für ruhende Daten](storage-service-encryption.md) verwenden.
...
**Gelten für das Ändern der Replikation eines Speicherkontos von GRS in LRS bestimmte Voraussetzungen?**

Nein. 

**Wie greife ich auf redundanten Azure Files-Speicher zu?**

Für den Zugriff auf redundanten Speicher ist georedundanter Speicher (Read-Access Geo-Redundant Storage, RA-GRS) mit Lesezugriff erforderlich. Azure Files unterstützt jedoch nur LRS und Standard-GRS ohne schreibgeschützten Zugriff. 

**Wie verschiebe ich Daten aus Storage Premium in Storage Standard?**

Gehen Sie dazu folgendermaßen vor:

1.  Erstellen Sie ein neues Standard-Speicherkonto. (Sie können auch ein vorhandenes Standard-Speicherkonto in Ihrem Abonnement verwenden.)
...
Es kann mehrere Stunden dauern, bis der Vorgang abgeschlossen ist.
...
**Wie stelle ich für eine Dateifreigabe auf Storage Premium um?**

Storage Premium ist für Azure-Dateifreigaben nicht zulässig.

**Wie aktualisiere ich von einem Standard-Speicherkonto auf ein Storage Premium-Konto? Wie stufe ich von einem Storage Premium-Konto auf ein Standard-Speicherkonto herunter?**

- Sie müssen das Zielspeicherkonto erstellen, Daten aus dem Quellkonto in das Zielkonto kopieren und dann das Quellkonto löschen.

- Sie können ein Tool wie AzCopy verwenden, um die Daten zu kopieren.

- Falls Sie außerdem virtuelle Computer besitzen, müssen vor dem Migrieren der Speicherkontodaten einige zusätzliche Schritte ausgeführt werden. Weitere Informationen finden Sie unter [Migrieren zu Azure Storage Premium (Nicht verwaltete Datenträger)](storage-migration-to-premium-storage.md).

**Wie verschiebe ich Ressourcen von einem klassischen Speicherkonto in ein Azure Resource Manager-Speicherkonto?**

1.  Sie können das Cmdlet „Move-AzureStorageAccount“ verwenden.

2.  Dieses Cmdlet beinhaltet mehrere Schritte (Überprüfen, Vorbereiten, Committen), und Sie können die Verschiebung vor der tatsächlichen Ausführung überprüfen.

3.  Falls Sie außerdem virtuelle Computer besitzen, müssen vor dem Migrieren der Speicherkontodaten einige zusätzliche Schritte ausgeführt werden. Weitere Informationen finden Sie unter [Migrieren von IaaS-Ressourcen aus dem klassischen Bereitstellungsmodell zu Azure Resource Manager mithilfe von Azure PowerShell](../..//virtual-machines/windows/migration-classic-resource-manager-ps.md).
...
-   Geben Sie einem Benutzer den primären oder sekundären Schlüssel für das Speicherkonto. Weitere Informationen finden Sie unter [Informationen zu Azure-Speicherkonten](storage-create-storage-account.md#manage-your-storage-account).
...
-   Haben Sie die 64-Bit-Version installiert, befindet es sich hier: **%ProgramFiles%\\Microsoft SDKs\\Azure\\AzCopy**.

**Wie greife ich bei einem replizierten Speicherkonto (etwa ZRS, GRS oder RA-GRS) auf Daten zu, die in der sekundären Region gespeichert sind?**

-   Wenn Sie zonenredundanten Speicher (Zone-Redundant Storage, ZRS) oder georedundanten Speicher (Geo-Redundant Storage, GRS) verwenden, können Sie nur im Falle eines Failovers auf Daten in der sekundären Region zugreifen. Weitere Informationen zum Failoverprozess finden Sie unter [Vorgehensweise beim Ausfall von Azure Storage](storage-disaster-recovery-guidance.md#what-to-expect-if-a-storage-failover-occurs).

-   Bei Verwendung von georedundantem Speicher mit Lesezugriff (Read-Access Geo-Redundant Storage, **RA-GRS**) können Sie jederzeit auf Daten in der sekundären Region zugreifen. Verwenden Sie hierzu eine der folgenden Methoden:  
    ...
    SAS-Token: Greifen Sie von einem Endpunkt mithilfe eines SAS-Tokens auf Daten zu. Weitere Informationen finden Sie unter [Verwenden von Shared Access Signatures (SAS)](storage-dotnet-shared-access-signature-part-1.md).
...
Wenn Sie Daten nur herunterladen möchten, ohne den Storage-Explorer oder eine ähnliche Anwendung verwenden zu müssen, können Sie unter Umständen ein SAS-Token nutzen. Weitere Informationen finden Sie unter [Verwenden von Shared Access Signatures (SAS)](storage-dotnet-shared-access-signature-part-1.md).
