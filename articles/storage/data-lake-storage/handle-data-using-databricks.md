...
![Azure Databricks mit Data Lake Storage Gen2 und SQL Data Warehouse](./media/handle-data-using-databricks/databricks-extract-transform-load-sql-datawarehouse.png "Azure Databricks mit Data Lake Storage Gen2 und SQL Data Warehouse")
...
1. Klicken Sie im Azure-Portal auf **Ressource erstellen** > **Analysen** > **Azure Databricks**.
    ...
    ![Databricks im Azure-Portal](./media/handle-data-using-databricks/azure-databricks-on-portal.png "Databricks im Azure-Portal")

2. Geben Sie unter **Azure Databricks-Dienst** die Werte für die Erstellung eines Databricks-Arbeitsbereichs an.

    ![Erstellen eines Azure Databricks-Arbeitsbereichs](./media/handle-data-using-databricks/create-databricks-workspace.png "Erstellen eines Azure Databricks-Arbeitsbereichs")
...
3. Die Kontoerstellung dauert einige Minuten. Während der Erstellung des Kontos wird im Portal auf der rechten Seite die Kachel **Submitting deployment for Azure Databricks** (Bereitstellung für Azure Databricks wird übermittelt) angezeigt. Möglicherweise müssen Sie im Dashboard nach rechts scrollen, um die Kachel zu sehen. Am oberen Bildschirmrand wird auch eine Statusanzeige angezeigt. Sie können den Status beider Bereiche beobachten.
    ...
    ![Kachel zur Bereitstellung von Databricks](./media/handle-data-using-databricks/databricks-deployment-tile.png "Kachel zur Bereitstellung von Databricks")
...
3. Geben Sie auf der Seite **Neuer Cluster** die erforderlichen Werte an, um einen Cluster zu erstellen.
    ...
    ![Erstellen eines Databricks-Spark-Clusters in Azure](./media/handle-data-using-databricks/create-databricks-spark-cluster.png "Erstellen eines Databricks-Spark-Clusters in Azure")
...
4. Geben Sie in die erste Zelle den folgenden Code ein, und führen Sie ihn aus:
...
1. Sollten Sie noch kein Konto für Data Lake Storage Gen2 erstellt haben, gehen Sie gemäß der Schnellstartanleitung vor, um ein Data Lake Storage Gen2-Konto zu erstellen.
2. Die Beispieldaten (**small_radio_json.json**) stehen im [Repository mit U-SQL-Beispielen und Problemverfolgung](https://github.com/Azure/usql/blob/master/Examples/Samples/Data/json/radiowebsite/small_radio_json.json) bereit. Laden Sie die JSON-Datei herunter, und notieren Sie sich den Speicherort der Datei.
3. Laden Sie die Daten in Ihr Speicherkonto hoch. Mit welcher Methode Sie die Daten in Ihr Speicherkonto hochladen, hängt davon ab, ob der hierarchische Namespacedienst (Hierarchical Namespace Service, HNS) aktiviert ist.

    Ist der hierarchischen Namespacedienst für Ihr ADLS Gen2-Konto aktiviert, können Sie Azure Data Factory, distp oder AzCopy (Version 10) für den Upload verwenden. Die AzCopy-Version 10 steht nur Vorschaukunden zur Verfügung. So verwenden Sie AzCopy über Cloud Shell:
...
6. Stellen Sie eine Verbindung mit der SQL-Datenbank her, und vergewissern Sie sich, dass eine Tabelle namens **SampleTable** vorhanden ist.
    ...
    ![Überprüfen der Beispieltabelle](./media/handle-data-using-databricks/verify-sample-table.png "Überprüfen der Beispieltabelle")
...
7. Führen Sie zur Überprüfung des Tabelleninhalts eine SELECT-Abfrage aus. Es sollten die gleichen Daten enthalten sein wie im Datenrahmen **renamedColumnsDF**.
    ...
    ![Überprüfen des Inhalts der Beispieltabelle](./media/handle-data-using-databricks/verify-sample-table-content.png "Überprüfen des Inhalts der Beispieltabelle")
...
![Beenden eines Databricks-Clusters](./media/handle-data-using-databricks/terminate-databricks-cluster.png "Beenden eines Databricks-Clusters")
