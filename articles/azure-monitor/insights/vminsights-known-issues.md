...
- Die Integritätsfunktion wird für alle VMs aktiviert, die mit dem Log Analytics-Arbeitsbereich verbunden sind, auch wenn dieser Vorgang von einer einzelnen VM aus gestartet und abgeschlossen wird.
- Wenn Sie nach der Deaktivierung der Überwachung für eine VM mithilfe der unterstützten Methoden das Onboarding wiederholen, sollte dies über den gleichen Arbeitsbereich erfolgen.  Wenn beim Anzeigen des Integritätsstatus für diese VM ein neuer Arbeitsbereich verwendet wird, wird möglicherweise ein anormales Verhalten angezeigt.
...
- Die Warnungszusammenfassung in Azure Monitor for VMs bezieht sich nur auf Warnungen, die für erkannte Integritätsprobleme auf überwachten Azure-VMs ausgelöst wurden.
...
- Die Zuweisung einer [Azur-Monitor-Aktionsgruppe](../../monitoring-and-diagnostics/monitoring-action-groups.md) für Zustandswarnungen ist vom Azur-Portal aus nicht möglich. Sie können die API nur für die Benachrichtigungseinstellungs-API verwenden, um eine Aktionsgruppe zu konfigurieren, die bei jeder Auslösung einer Integritätswarnung ausgelöst wird. Derzeit können Aktionsgruppen einer VM zugewiesen werden, sodass alle *Integritätswarnungen*, die für die VM ausgelöst wurden, die gleiche(n) Aktionsgruppe(n) ausgelöst haben. Es gibt kein Konzept für eine separate Aktionsgruppe für jede Integritätswarnregel, wie bei herkömmlichen Azure-Warnungen. Darüber hinaus werden nur Aktionsgruppen unterstützt, die so konfiguriert sind, dass sie durch Senden einer E-Mail oder SMS benachrichtigt werden, wenn Integritätswarnungen ausgelöst werden. 
