...
| Standardmäßig sicher | Standard verwenden, eingehend geschlossen für öffentliche IP- und Load Balancer-Endpunkte, und es muss eine Netzwerksicherheitsgruppe verwendet werden, über die explizit eine Whitelist verwendet wird, damit Datenverkehr möglich ist. | Standardmäßig geöffnet, Netzwerksicherheitsgruppe ist optional |
...
| [Regeln für ausgehenden Datenverkehr](../articles/load-balancer/load-balancer-outbound-rules-overview.md) | Deklarative NAT-Konfiguration für ausgehenden Datenverkehr, einschließlich der öffentlichen IP-Adresse oder des Präfix der öffentlichen IP-Adresse, konfigurierbares Leerlauftimeout für ausgehenden Datenverkehr, benutzerdefinierte SNAT-Portzuordnung. | Nicht verfügbar |
...
| Preise | Berechnung anhand der Anzahl der Regeln, der Daten, die eingehend oder ausgehend verarbeitet werden, die der Ressource zugeordnet sind.  | Keine Berechnung |
|  |  |  |
